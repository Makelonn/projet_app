...
class CharacterCreator():
    ...
    def __init__(self):
        ...
        # --- Columns ---
        self.image_col = [
            [sg.Text('Image du personnage', size=(17, 1), justification='center')], 
            [sg.Image('./assets/2.png', size=(200, 200), key='-IMAGE-')],
            [sg.FileBrowse(key='-IMGBROWSE-', file_types=IMG_TYPES, enable_events=True)],
        ]
        self.base_col = [
            ...
            [sg.Text("Race : ", size=(13, 1)), sg.InputCombo(DD_RACES, default_value="Votre race ici", size=(15, 1))],
            [sg.Text("Alignement : ", size=(13, 1)), sg.InputCombo(DD_ALIGNMENT, default_value="Votre alignement ici", size=(15, 1))],
            [sg.Text("Classe :", size=(13, 1)), sg.InputCombo(DD_CLASSES, default_value="Votre classe ici", size=(15, 1))],
            [sg.Text("Niveau :", size=(12, 1)), sg.Input(key='-LVL-', enable_events=True, size=(15, 1))], 
            [sg.Text("Points d'expérience : ",  size=(12, 1)), sg.Input(key='-XP-', size=(15, 1), enable_events=True)],
        ]
        self.appearance_col = [
            [sg.Text("Âge :", size=(10, 1)), sg.Input(key='-AGE-', size=(10, 1)), 
            sg.Text('Taille (cm) :', size=(10, 1)), sg.Spin(values=[150, 155, 160, 165, 170, 175, 180, 185, 190, 195], initial_value=175, size=(10, 1)), 
            ...
            sg.Text("Yeux : ", size=(9, 1)), sg.InputCombo(('Bleus', 'Verts', 'Bruns', 'Gris', 'Rouges', 'Sans'), size=(10, 1), default_value="Bleus"), 
        ...
        self.stats = [
            [sg.Text("Force :", size=(10, 1)), sg.Slider((0, 20), orientation='h', key='-SLIDER_F-', enable_events=True, disable_number_display=True), sg.Text('0', size=(4, 1), key='-S_F-')],
                [sg.Text("Intelligence :", size=(10, 1)), sg.Slider((0, 20), orientation='h', key='-SLIDER_I-', enable_events=True, disable_number_display=True), sg.Text('0', size=(4, 1), key='-S_I-')],
                [sg.Text("Dextérité :", size=(10, 1)), sg.Slider((0, 20), orientation='h', key='-SLIDER_D-', enable_events=True, disable_number_display=True), sg.Text('0', size=(4, 1), key='-S_D-')],
                [sg.Text("Sagesse :", size=(10, 1)), sg.Slider((0, 20), orientation='h', key='-SLIDER_S-', enable_events=True, disable_number_display=True), sg.Text('0', size=(4, 1), key='-S_S-')],
                [sg.Text("Constitution :", size=(10, 1)), sg.Slider((0, 20), orientation='h', key='-SLIDER_CO-', enable_events=True, disable_number_display=True), sg.Text('0', size=(4, 1), key='-S_CO-')],
                [sg.Text("Charisme :", size=(10, 1)), sg.Slider((0, 20), orientation='h', key='-SLIDER_CH-', enable_events=True, disable_number_display=True), sg.Text('0', size=(4, 1), key='-S_CH-')]
        ]
        self.statistics_col = [
                [sg.Column(self.stats, element_justification='left', justification='left', expand_x=True), sg.Column(self.canvas_col, element_justification='right', justification='right', expand_x=True)]
        ]
        ...
        self.layout = [
            ...
            [sg.HorizontalSeparator(pad=((0, 0), (7, 7)))],
            [sg.Column(self.statistics_col, justification='center', expand_x=True)]
        ]

        # Window
        self.window = sg.Window('Créateur de personnage',
                                self.layout, margins=(5, 5), font=(FONT, 14), finalize=True)

        # --- Expanding things ---
        self.expand_objets()

        # -- Graph --
        self.stats_keys= ['-S_F-', '-S_I-', '-S_D-', '-S_S-', '-S_CO-', '-S_CH-']
        self.slider_keys = ['-SLIDER_F-', '-SLIDER_I-', '-SLIDER_D-', '-SLIDER_S-', '-SLIDER_CO-', '-SLIDER_CH-']
    ...
    def initialize_graph(self):
        ...
        self.fcg = FigureCanvasTkAgg(fig, master=self.window['-CANVAS-'].TKCanvas)
        self.fcg.draw()
    ...
    def update_graph(self, values):
        ...
        for key in range(len(self.stats_keys)):
            self.window[self.stats_keys[key]].update(int(values[self.slider_keys[key]]))
    ...
    def loop(self):
        ...
        while True:
            ...
            if event in ('-XP-', '-LVL-', '-AGE-'):
                if event == '-XP-' and values['-XP-'][-1] not in ('0123456789-'):
                    self.window['-XP-'].update(values['-XP-'][:-1])
                if event == '-LVL-' and values['-LVL-'][-1] not in ('0123456789-'):
                    self.window['-LVL-'].update(values['-LVL-'][:-1])
                if event == '-AGE-' and values['-AGE-'][-1] not in ('0123456789-'):
