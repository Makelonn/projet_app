...
Entity(model='plane', position=level.water.position, scale=9999,
       color=color.color(160, 1, .8, .5), double_sided=True)
...
def open_chest():
    if distance_xz(player.position, level.chest.position) < 6:
        level.chest_lid.animate(
            'rotation_x', level.chest_lid.rotation_x + 120, duration=.2)
...
def input(key):
    ...
    if level.bow.enabled and key == 'left mouse down':
        player.arrow = duplicate(
            level.arrow, world_parent=level.bow, position=Vec3(-.2, 0, 0), rotation=Vec3(0, 0, 0))
        player.arrow.animate('position', player.arrow.position +
                             Vec3(0, 0, -2), duration=.2, curve=curve.linear)
    ...
    if level.bow.enabled and key == 'left mouse up':
        if mouse.hovered_entity and mouse.hovered_entity.visible:
            ...
            player.arrow.world_parent = scene
            player.arrow.animate('position', player.arrow.world_position+(
                player.arrow.forward*100), .5, curve=curve.linear, interrupt='kill')
...
def open_gate():
    ...
    level.gate_001.animate_position(
        level.gate_001.position+(level.gate_001.right)*10, duration=5, curve=curve.linear)
