...
class Ball:
    ...
    def update(self, delta: float):
        ...
        ):  # see if hit top or bottom of play area. If so, reverse y direction
            self.vy = -self.vy
            self.current_y = BALL_RADIUS
    ...
    def position_to_current(self):
        self.graph.relocate_figure(
            self.id, self.current_x - BALL_RADIUS, self.current_y - BALL_RADIUS
        )
...
def pong():
    ...
    main_menu_layout = [
        [sg.Text("Pong", font="Courier 40", justification="center", size=(None, 1))],
    ...
    layout = [
        [
            sg.pin(sg.Column(main_menu_layout, key="-MAIN_MENU-", size=GAMEPLAY_SIZE)),
    ...
    bat_2 = Bat(
        graph_elem, "blue", GAMEPLAY_SIZE[0] - 30 - BAT_SIZE[0], GAMEPLAY_SIZE[1]
    )
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, "-QUIT-"):
            ...
            if event == "+KEY+":
                ...
                if window.user_bind_event.keycode in [
                    player1_up_keycode,
                    player1_down_keycode,
                ]:
                    bat_1.stop()
                elif window.user_bind_event.keycode in [
                    player2_up_keycode,
                    player2_down_keycode,
                ]:
                    bat_2.stop()
